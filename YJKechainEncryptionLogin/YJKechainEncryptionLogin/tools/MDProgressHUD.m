...
#define MDMainThreadAssert() NSAssert([NSThread isMainThread], @"MDProgressHUD needs to be accessed on the main thread.");

CGFloat const MDProgressMaxOffset = 1000000.f;

static const CGFloat MDDefaultPadding = 4.f;
static const CGFloat MDDefaultLabelFontSize = 16.f;
static const CGFloat MDDefaultDetailsLabelFontSize = 12.f;


@interface MDProgressHUD ()
...
@interface MDProgressHUDRoundedButton : UIButton
@end


@implementation MDProgressHUD

#pragma mark - Class methods

+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    MDProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    MDProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (MDProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            MDProgressHUD *hud = (MDProgressHUD *)subview;
...
- (void)commonInit {
    ...
    _animationType = MDProgressHUDAnimationFade;
    _mode = MDProgressHUDModeIndeterminate;
    _margin = 20.0f;
...
- (void)animateIn:(BOOL)animatingIn withType:(MDProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == MDProgressHUDAnimationZoom) {
        type = animatingIn ? MDProgressHUDAnimationZoomIn : MDProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == MDProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == MDProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == MDProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == MDProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
...
- (void)done {
    ...
    MDProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<MDProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    MDBackgroundView *backgroundView = [[MDBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = MDProgressHUDBackgroundStyleSolidColor;
    ...
    MDBackgroundView *bezelView = [MDBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:MDDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:MDDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;

    UIButton *button = [MDProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:MDDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[MDRoundProgressView class]];

    MDProgressHUDMode mode = self.mode;
    if (mode == MDProgressHUDModeIndeterminate) {
    ...
    else if (mode == MDProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[MDBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == MDProgressHUDModeDeterminate || mode == MDProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[MDRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == MDProgressHUDModeAnnularDeterminate) {
            [(MDRoundProgressView *)indicator setAnnular:YES];
        }
    } 
    else if (mode == MDProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == MDProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
- (void)updateViewsForColor:(UIColor *)color {
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[MDProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[MDProgressHUD class]]];
#endif
        
        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[MDRoundProgressView class]]) {
        MDRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [MDRoundProgressView appearanceWhenContainedIn:[MDProgressHUD class], nil];
#else
        appearance = [MDRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[MDProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((MDRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((MDRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[MDBarProgressView class]]) {
        MDBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [MDBarProgressView appearanceWhenContainedIn:[MDProgressHUD class], nil];
#else
        appearance = [MDBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[MDProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((MDBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((MDBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    MDBackgroundView *bezelView = self.bezelView;
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? MDDefaultPadding : 0.f;
...
- (void)setMode:(MDProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == MDProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation MDRoundProgressView
...
@implementation MDBarProgressView
...
@implementation MDBackgroundView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _style = MDProgressHUDBackgroundStyleBlur;
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
            ...
            _style = MDProgressHUDBackgroundStyleSolidColor;
...
- (void)setStyle:(MDProgressHUDBackgroundStyle)style {
    if (style == MDProgressHUDBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_7_0) {
        style = MDProgressHUDBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    MDProgressHUDBackgroundStyle style = self.style;
    if (style == MDProgressHUDBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == MDProgressHUDBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation MDProgressHUDRoundedButton
